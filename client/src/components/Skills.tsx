...
const professionalSoftware: Software[] = [
  { 
    ...
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-[#aaff00]" viewBox="0 0 24 24" fill="currentColor">
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-2">
        ...
        <span className="text-sm text-[#aaff00]">{skill.level}</span>
      </div>
...
const SoftwareIcon: React.FC<{ software: Software; index: number }> = ({ software, index }) => {
  ...
  return (
    ...
    >
      <div className="w-16 h-16 bg-[#aaff00] bg-opacity-10 dark:bg-opacity-20 rounded-full flex items-center justify-center mb-3">
        {software.icon}
      </div>
...
const Skills: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-20 md:py-32 bg-gray-100 dark:bg-black">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          <motion.div 
            ref={techRef}
            className={`bg-white dark:bg-zinc-950 rounded-xl p-8 shadow-lg reveal ${techActive ? 'active' : ''}`}
            initial={{ opacity: 0, y: 30 }}
            animate={techActive ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.6, ease: "easeOut" }}
          >
            <h3 className="text-2xl font-bold mb-8 flex items-center">
              <span className="w-10 h-10 flex items-center justify-center bg-[#aaff00] text-white rounded-full mr-3">
          ...
          <motion.div 
            ref={softwareRef}
            className={`bg-white dark:bg-zinc-950 rounded-xl p-8 shadow-lg reveal ${softwareActive ? 'active' : ''}`}
            initial={{ opacity: 0, y: 30 }}
